  ...
  class Facts
    ...
    class PuppetDB
      ...
      def self.fact_retriever(options = {}, node)
        ...
        return obj_to_return if puppetdb_api_version < 4
        ...
        (retries + 1).times do
          begin
            ...
            exception_class = OctocatalogDiff::Errors::FactSourceError
            exception_message = "Fact packages retrieval failed (#{exc.class}) (#{exc.message})"
          rescue OctocatalogDiff::Errors::PuppetDBNodeNotFoundError
            packages = {}
          rescue OctocatalogDiff::Errors::PuppetDBGenericError => exc
            exception_class = OctocatalogDiff::Errors::FactRetrievalError
            exception_message = "Fact packages retrieval failed for node #{node} from PuppetDB (#{exc.message})"
          end
        end
